...
> **Last updated**: 2026-03-30 — Cập nhật sau peer review
...
> [!NOTE]
> Các quyết định dưới đây được chốt sau khi review phản biện từ AI reviewer.

| # | Phản biện | Quyết định | Tác động |
|---|-----------|------------|----------|
| 1 | Lệch data giữa 2 API (sections vs rows) | Chấp nhận — thêm `lastCalculatedAt` timestamp vào sections response, FE hiển thị thời điểm | Sửa DTO |
| 2 | Breaking change /rows (bỏ formula fields) | Chấp nhận break luôn — chưa có client production | Không cần API versioning |
| 3 | Stringly-typed fields dễ sai (RowType, Position...) | **Đồng ý** — thêm CHECK constraints ở DB + C# constants | Sửa migration + thêm constants class |
| 4 | Seed ID cứng (1..6) dễ vỡ | Chấp nhận tạm — pre-release, reset DB được | Không sửa |
| 5 | Formula scope (book vs group level) chưa rõ | Chấp nhận — clarify lúc implement Task 3.3 | Thêm note |
| 6 | BusinessType mapping cần chuẩn hóa | Đã có migration 073 — đây là data sync issue, không phải architecture issue | Không sửa |
...
CREATE TABLE TemplateRowDefinitions (
    ...
    -- ═══ CHECK CONSTRAINTS (peer review: giảm lỗi stringly-typed) ═══
...
### Task 1.4 — C# Constants cho RowType, Position, SectionType (Peer Review)

> [!TIP]
> Thay vì dùng magic strings rải rác trong code, tập trung vào 1 constants class. Compile-time safety + IntelliSense.

#### [NEW] [RowDefinitionConstants.cs](file:///d:/Semester-9/bizflow-project/BizFlow-BE-Service/bizflow-platform/BizFlow.Domain/Constants/RowDefinitionConstants.cs)

```csharp
namespace BizFlow.Domain.Constants;

/// <summary>
/// Constants for TemplateRowDefinition fields — matches DB CHECK constraints.
/// </summary>
...
**Sử dụng trong code** (thay vì magic strings):

```csharp
// TRƯỚC:
if (rowDef.RowType == "tax_line") ...

// SAU:
using static BizFlow.Domain.Constants.RowDefinitionConstants;
if (rowDef.RowType == RowType.TaxLine) ...
```
...
> [!WARNING]
> **Formula scope clarification (from peer review #5)**:
> - Formulas hiện tại (`FormulaEngine`) tính ở **book-level** (toàn sổ).
> - `RenderSectionsAsync` cần tính subtotal ở **group-level** (per BusinessType).
> - **Giải pháp**: Subtotal per group = query `SUM(Amount)` grouped by BusinessTypeId (SQL aggregation), KHÔNG dùng FormulaEngine. FormulaEngine chỉ dùng cho grand total / cross-group formulas (ví dụ: `S2A_PIT` cần total revenue toàn location).
> - Tránh duplicate logic bằng cách: group-level = SQL aggregate, book-level = FormulaEngine.
...
public class BookSectionsResponse
{
    ...
    /// <summary>
    /// Thời điểm dữ liệu được tính toán. Client hiển thị "Dữ liệu cập nhật lúc: ...".
    /// Giúp user nhận biết nếu data đã thay đổi sau khi gọi /sections.
    /// (Peer review #1: giải pháp cho data consistency giữa 2 API)
    /// </summary>
...
| # | File | Layer | Mô tả |
|---|------|-------|-------|
| 1 | `database/migrations/079_create_template_row_definitions.sql` | DB | Tạo bảng + CHECK constraints |
...
| 4 | `BizFlow.Domain/Constants/RowDefinitionConstants.cs` | Domain | **[NEW]** Enum-like constants cho RowType, Position, SectionType, TaxType |
...
> [!NOTE]
> Không còn câu hỏi mở — tất cả decisions đã chốt qua discussion + peer review.
...
- ✅ Breaking change /rows — không cần backward compatibility (peer review #2)
- ✅ CHECK constraints + C# constants cho stringly-typed fields (peer review #3)
- ✅ `lastCalculatedAt` timestamp trong sections response (peer review #1)
- ✅ Formula scope: group-level = SQL aggregate, book-level = FormulaEngine (peer review #5)
